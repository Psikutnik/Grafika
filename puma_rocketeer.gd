...
@onready var barrels = [$RocketBarrel, $RocketBarrel2, $RocketBarrel3,$RocketBarrel4, $RocketBarrel5, $RocketBarrel6, $RocketBarrel7]
...
func _hit_finished():
	...
	for barrel in barrels:
			var offset = Vector3(
			randf_range(-1.0, 1.0),  # Rozrzut w osi X
			randf_range(-2.0, 2.0),  # Rozrzut w osi Y
			randf_range(-1.0, 1.0)   # Rozrzut w osi Z
			)
			barrel.look_at(target_pos + offset, Vector3.UP)
			var bullet_instance = bullet.instantiate()  # Nowa instancja dla każdej lufy
			bullet_instance.global_transform = barrel.global_transform
			get_tree().root.add_child(bullet_instance)  # Dodajemy do sceny
